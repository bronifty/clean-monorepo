...
export type DataRecordType = { name: string; author: string }; // for everything except http.get
export type RequestDTO = { result: DataRecordType[] }; // for the http.get
export type UnsubscribeFunction = () => void; // for the repo.subscribe
type ResultMessage = {
  result: "success";
...
export class Database implements IDatabase {
  ...
  get = async (): Promise<ResultMessage> => {
    try {
  ...
  init = async (): Promise<ResultMessage> => {
    try {
      this.data = [
...
export interface IHttpGateway {
  ...
  post(path: string, requestDto: RequestDTO): Promise<ResultMessage>;
...
class Repository implements IRepository {
  ...
  init = async (): Promise<ResultMessage> => {
    try {
      console.log(
        `Repository.init(); await this.httpGateway.init(); await this.get()`
      );
      await this.httpGateway.init();
      await this.get();
      return Promise.resolve({ result: "success" });
    } catch (error) {
      throw new Error("custom error");
...
export function BooksComposer({ observable }: BooksComposerProps) {
  ...
  React.useEffect(() => {
    ...
    load();
    // const dataSubscription = observable.subscribe((value) => {
    //   setDataValue(value);
    // });
